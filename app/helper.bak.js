 ...
 * @param {*} orgName 组织名称
 */
function isSysNode(orgName) {
	var nodes = hfc.getConfigSetting('platCCInstallNode');
	for (var i = 0; i < nodes.length; i++) {
		if (nodes[i].orgName == orgName) {
			return true;
		}
	}
	return false;
...
function loadConfigWithOrg(userorg) {
	var networkConfig = "";
	let orgConfig = userorg + '-connection-profile-path';
	...
	if (isSysNode(userorg)) {//发果是系统平台组织
		networkConfig = 'network-connection-profile-path'
	} else {
		networkConfig = userorg + '-network-connection-profile-path'
		file = userorg + "-" + file;
	}
	hfc.setConfigSetting(networkConfig, path.join(path.resolve(__dirname, '..'), 'artifacts', file));
	
	return { networkConfig: networkConfig, orgConfig: orgConfig, file: file};
}

async function getClientForOrg (userorg, username) {
	// get a udo client loaded with a connection profile for this org
	var configObj = loadConfigWithOrg(userorg);

	// build a client context and load it with a connection profile
	// lets only load the network settings and save the client for later
	let client = hfc.loadFromConfig(hfc.getConfigSetting(configObj.networkConfig));
	...
	client.loadFromConfig(hfc.getConfigSetting(configObj.orgConfig));
	...
	if(username) {
		...
		if(!user) {
			var retObj = await loadUser(username, userorg, client, configObj.networkConfig);
...
var loadUser = async function (username, userOrg, client,networkConfig) {
	try {
		if (!networkConfig) {
			var configObj = loadConfigWithOrg(userOrg);
			networkConfig = configObj.networkConfig;
		}
		let configPathFile = hfc.getConfigSetting(networkConfig);
...
var signGasData = function (userOrg, data) {
	var configObj = loadConfigWithOrg(userOrg);
	let configPathFile = hfc.getConfigSetting(configObj.networkConfig);
...
var parseMsg = function (details) {
	if (details.indexOf("chaincode error") >= 0) {
		var regex = "\\((.+?)\\)";
