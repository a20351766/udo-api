angular.module('walletApp', []).controller('walletCtrl', function ($scope, $http) { 
    ...
    $scope.apply = function () {
        ...
        var origin = { 
            ...
            remark: $scope.remark,
            contract_name: $scope.contract_name,
            contract_tel: $scope.contract_tel,
            pubKey: pubKey
        };
        ...
        $http({
            ...
            method: 'POST',
            data: { rawData: hexStr},
            url: './v1/wallet/udo_chainEnterUpdate'
        }).then(function successCallback(response) {
            layer.close(layerIndex);
            var data = response.data;
            $scope.result = JSON.stringify(data);
            if(data.status) {
                layer.msg('修改成功。', { icon: 6, shade: [0.6, '#393D49'], scrollbar: false, time: 3000 });
            } else {
